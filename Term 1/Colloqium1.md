...
4. *Линейно-логарифмический*:  **O(n*log(n))**.
  
5. *Квадратичный*: **O(n^2)**.

6. *Кубический*: **O(n^3)**.

7. *Полиноминальный*: **O(n^m)**.

8. *Экспоненциальный* **O(t^p(n))**, **t** — константа, **p(n)** — некоторая полиноминальная функция.

9. *Факториальный*: **O(n!)**. Обладает наибольшей временной сложностью среди всех известных типов.
...
| По памяти  | O(n)          | O(n)           | O(n)          |
  ...
  <br>По времени: n тратится на слияние, log(n) на разбиениюе через вызов рекурсии. 
